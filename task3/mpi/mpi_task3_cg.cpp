...
void Process::init_processor_config(){
    ...
    g = new double [(size_x + 2) * (size_y + 2)];
    g_pr = new double [(size_x + 2) * (size_y + 2)];
...
double Process::solve_iteration(){
    ...
    double diff, diff_1, diff_2, diff_local_1, diff_local_2;
    double alpha_local_1, alpha_local_2, beta_local_1, beta_local_2;
    ...
    // alpha_k = norm(gk)^2 / (Ark, rk)
    alpha_local_1 = _inner_product(g, g, size_x, size_y, i_x, j_y, M, N, h1, h2);
    alpha_local_2 = _inner_product(Ar, r, size_x, size_y, i_x, j_y, M, N, h1, h2);
    // (input data, output data, data size, data type, operation type, communicator)
    MPI_Allreduce(&alpha_local_1, &alpha_1, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&alpha_local_2, &alpha_2, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    ...
    for(int i = 0; i <= size_x; i++)
        for(int j = 0; j <= size_y; j++)
            w[i * (size_y + 2) + j] = w[i * (size_y + 2) + j] + alpha * r[i * (size_y + 2) + j];

    // store current g
    for(int i = 0; i <= size_x; i++)
        for(int j = 0; j <= size_y; j++)
            g_pr[i * (size_y + 2) + j] = g[i * (size_y + 2) + j];
        
    // update g: g_k+1 = g_k - alpha_kAr_k
    for(int i = 0; i <= size_x; i++)
        for(int j = 0; j <= size_y; j++)
            g[i * (size_y + 2) + j] = g[i * (size_y + 2) + j] - alpha * Ar[i * (size_y + 2) + j];
        
    // norm(g_k+1)^2 / norm(g_k)^2
    beta_local_1 = _inner_product(g, g, size_x, size_y, i_x, j_y, M, N, h1, h2);
    beta_local_2 = _inner_product(g_pr, g_pr, size_x, size_y, i_x, j_y, M, N, h1, h2);
    // (input data, output data, data size, data type, operation type, communicator)
    MPI_Allreduce(&beta_local_1, &beta_1, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&beta_local_2, &beta_2, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    ...
    for(int i = 0; i <= size_x; i++)
        for(int j = 0; j <= size_y; j++)
            r[i * (size_y + 2) + j] = g[i * (size_y + 2) + j] + beta * r[i * (size_y + 2) + j];
        
    diff_local_1 = _inner_product(g, g, size_x, size_y, i_x, j_y, M, N, h1, h2);
    diff_local_2 = _inner_product(B, B, size_x, size_y, i_x, j_y, M, N, h1, h2);
    MPI_Allreduce(&diff_local_1, &diff_1, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&diff_local_2, &diff_2, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
void Process::solve(){
    ...
    for(int i = 0; i <= size_x; i++)
        for(int j = 0; j <= size_y; j++)
            g[i * (size_y + 2) + j] = B[i * (size_y + 2) + j] - Aw[i * (size_y + 2) + j];

    // init r： r = B
    for(int i = 0; i <= size_x; i++)
        for(int j = 0; j <= size_y; j++)
            r[i * (size_y + 2) + j] = g[i * (size_y + 2) + j];

    // norm_B
    norm_B = norm(B, size_x, size_y, i_x, j_y, M, N, h1, h2);
    ...
    do{
        ...
        if (rank == 0 && iter % PRINT_FREQ == 0) cout << "iter: " << iter << ", err_norm: " << diff << "\n";
    ...
    if (rank == 0){
        cout << "Finished !!!" << endl;
        cout << "total_iter: " << iter <<", final_err_norm: " << diff << "\n";
