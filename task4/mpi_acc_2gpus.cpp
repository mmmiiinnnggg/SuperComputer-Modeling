...
private:
    ...
    void exchange_data_2proc(double *_w);
...
void Process::exchange_data_2proc(double *_w){
    // with only two process-gpus 
    ...
    if (coords[0] == 0 && coords[1] == 0){
        ...
        for (int i = 1; i <= size_y; i++){
            // w_r_bound[i - 1] = _w[size_x * (size_y + 2) + i];
            w_l_bound[i - 1] = _w[1 * (size_y + 2) + i];
        }
        }
    }
    ...
    if (rank == 0){
        ...
        MPI_Isend(w_r_bound, size_y, MPI_DOUBLE, 1, TAG_X, MPI_COMM_WORLD, &request);
    }

    // right process rank 1 recv right bound values to left buffer
    else {
        // right process recv values and put in left buffer
        MPI_Irecv(w_l_buffer, size_y, MPI_DOUBLE, 0, TAG_X, MPI_COMM_WORLD, &request);
    ...
    if (rank == 0){
        ...
        MPI_Irecv(w_r_buffer, size_y, MPI_DOUBLE, 1, TAG_Y, MPI_COMM_WORLD, &request);
        MPI_Wait(&request, &status);
        ...
        MPI_Isend(w_l_bound, size_y, MPI_DOUBLE, 0, TAG_Y, MPI_COMM_WORLD, &request);
...
void Process::solve(){
    ...
    // exchange_data(B);
    // #pragma acc update device(B[0:_size])
    exchange_data_2proc(B);
