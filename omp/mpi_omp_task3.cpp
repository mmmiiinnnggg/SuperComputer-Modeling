...
double _inner_product(double *w0, double *w1, int size_x, int size_y, int i_x, int j_y, int M, int N, double h1, double h2)
{
    ...
    int i, j, _i, _j;
    #pragma omp parallel for default(shared) private(i, j, _i, _j) schedule(dynamic) reduction(+:res)
    for (i = 1; i <= size_x; i++)
    {
        for (j = 1; j <= size_y; j++)
        {
...
void init_B(double *B, int M, int N, int size_x, int size_y, int i_x, int j_y, double h1, double h2){
    int i, j, _i, _j;
    #pragma omp parallel for default(shared) private(i, j, _i, _j) schedule(dynamic)
    for (i = 1; i <= size_x; i++)
        for (j = 1; j <= size_y; j++){
...
double Process::solve_iteration(){
    ...
    #pragma omp parallel default(shared) 
    {
        ...
        // calculate diff
        // vector_diff(diff_w_and_w_pr, w, w_pr, size_x, size_y); -> change to full form
        #pragma omp for private(i, j) schedule(dynamic)
        for(i = 0; i <= size_x + 1; i++){
            for(j = 0; j <= size_y + 1; j++){
        ...
        // diff_local = norm(diff_w_and_w_pr, size_x, size_y, i_x, j_y, M, N, h1, h2); -> change to full form
        // diff_local = 0;
        int _i, _j;
        #pragma omp for private(i, j, _i, _j) schedule(dynamic) reduction(+:diff_local)
        for (i = 1; i <= size_x; i++)
        {
            for (j = 1; j <= size_y; j++)
            {
                ...
                // corner points
                if (_i == 0 && _j == 0 || _i == 0 && _j == N || _i == M && _j == 0 || _i == M && _j == N)
                    diff_local += 0.25 * diff_w_and_w_pr[i * (size_y + 2) + j] * diff_w_and_w_pr[i * (size_y + 2) + j];
                // inner points
                else if (_i >= 1 && _i <= M - 1 && _j >= 1 && _j <= N - 1)
                    diff_local += 1.0 * diff_w_and_w_pr[i * (size_y + 2) + j] * diff_w_and_w_pr[i * (size_y + 2) + j];
                // bound points
                else diff_local += 0.5 * diff_w_and_w_pr[i * (size_y + 2) + j] * diff_w_and_w_pr[i * (size_y + 2) + j];
            }
        }
    }

    diff_local = diff_local * h1 * h2;
